...
class ProductCore extends ObjectModel
{
	...
	*/
	public static function getNewProducts($id_lang, $page_number = 0, $nb_products = 10, $count = false, $order_by = null, $order_way = null, Context $context = null)
	{
		...
		if (Group::isFeatureActive())//Заходит
		{			
		...
		$sql->limit($nb_products, $page_number * $nb_products);//$nb_products то самое кол-во новостей, которое выведется
		//print_r($sql->limit($nb_products, $page_number * $nb_products));
		//echo('<br />');
		if (Combination::isFeatureActive())//Заходит, тут появляется 8 число выводимых новых товаров  [limit] => Array ( [offset] => 0 [limit] => 8 ) ) ) 
		{
			//print_r($sql);
		//echo('<br />');
		...
		foreach ($result as $row)
			...
			//print_r($result); 8 новых товаров
			//echo('<br />');
	...
	*/
    ...
    public   function getProductCombinationsGroups($id_product) {
        $groups = array();
        $context = Context::getContext();
        ...
        foreach ($attributes_groups as $k => $row)
        {
            ...
            if (!Product::isAvailableWhenOutOfStock($this->out_of_stock) && Configuration::get('PS_DISP_UNAVAILABLE_ATTR') == 0)
            {
