<?php
/*
* 2007-2015 PrestaShop _tat_ добавлены функции <font>getSubCategoriesMatrassHome</font> (для добавления категори с парант 3 (матрасов) на домашнюю страницу и getSubCategoriesHit для вывода хитов (категория 3) на страницы родительских категорий)
*
* NOTICE OF LICENSE
*
...
class CategoryCore extends ObjectModel
{
	...
	//Для вывода категорий на главную
	public function getSubCategoriesMatrassHome($id_lang, $active = true)
	{
		$sql_groups_where = '';
		$sql_groups_join = '';
		if (Group::isFeatureActive())
	...
	//Для вывода хитов в родительские категории
	public function getSubCategoriesHit($id_lang, $active = true)
	{
		$sql_groups_where = '';
		$sql_groups_join = '';
		if (Group::isFeatureActive())
	...
	public function getProducts($id_lang, $p, $n, $order_by = null, $order_way = null, $get_total = false, $active = true, $random = false, $random_number_products = 1, $check_access = true, Context $context = null)
	{
		...
		$sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity'.(Combination::isFeatureActive() ? ', MAX(product_attribute_shop.id_product_attribute) id_product_attribute, MAX(product_attribute_shop.minimal_quantity) AS product_attribute_minimal_quantity' : '').', pl.`description`, pl.`description_short`, pl.`available_now`,
					...
					INTERVAL '.(Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).'
						DAY)) > 0 AS new,
						 IFNULL(MIN(product_attribute_shop.price),product_shop.price) AS orderprice,
						 IFNULL(MIN(product_attribute_shop.price),product_shop.price) AS price
