...
if ($_POST) { // если передан массив POST
	...
	$mail = htmlspecialchars($_POST["mail"]);//email отправителя	
	$email = htmlspecialchars($_POST["email"]);//email владельца сайта
	$subject = htmlspecialchars($_POST["subject"]);//тема
	$tel = htmlspecialchars($_POST["tel"]);//телефон
	$namesymbol = htmlspecialchars($_POST["namesymbol"]);//телефон
	$telsymbol = htmlspecialchars($_POST["telsymbol"]);//телефон
	$messagesymbol = htmlspecialchars($_POST["messagesymbol"]);//телефон
	...
	$emailgo->from_name= 'perrino-shop.ru/';//Оборачивает адрес с которого письмо
	$emailgo->to_email= $email; // кому (владельцу сайта)
	$emailgo->to_name= $name;//Имя отправителя
	$emailgo->subject= $subject; // тема	
	$emailgo->tel= $tel; // телефон	
	$emailgo->namesymbol= $namesymbol; // Имя подпись	
	$emailgo->telsymbol= $telsymbol; // телкфон	подпись	
	$emailgo->messagesymbol= $messagesymbol; // сообщение подпись		
	$emailgo->body= $namesymbol.$name."\r\n".$messagesymbol.$message."\r\n".$telsymbol.$tel."\r\n email: ".$mail; // сообщение	
