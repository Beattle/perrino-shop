<?php
/*
* 2007-2014 PrestaShop _tat_ Выводит уценённые товары. Изменяла функцию hookRightColumn, которая выводила слева один случайный товар, сейчас кол-во указанное в конфигурации 
*
* NOTICE OF LICENSE
*
...
class BlockSpecials extends Module
{
	...
	{
		if (Configuration::get('PS_CATALOG_MODE'))//Если каталог-режим, то этого не надо?
			return;
		// Нужно для выводва случайных комбинаций товаров
		$random = date('Ymd').'|'.round(rand(1, max(Configuration::get('BLOCKSPECIALS_NB_CACHES'), 1)));
		//echo (Configuration::get('BLOCKSPECIALS_NB_CACHES'))

		if (!$this->isCached('blockspecials.tpl', $this->getCacheId('blockspecials|'.date('Ymd'))))
		{
			if (!$specials = Product::getPricesDrop((int)$params['cookie']->id_lang, 0, 100, false))//Так я передала в шаблон все товары. Но выведу там не все, а начиная с определённого "полуслучайного"
			//if (!$specials = Product::getPricesDrop((int)$params['cookie']->id_lang, 0, Configuration::get('BLOCKSPECIALS_SPECIALS_NBR')))_tat_ так я могу вывести количество товаров, указанных в конфигурации админки (шесть))
			//if (!$specials = Product::getPricesDrop((int)$params['cookie']->id_lang, 0, 10))
                return;
		...
		return $this->display(__FILE__, 'blockspecials.tpl', $this->getCacheId('blockspecials|'.date('Ymd')));
		//echo (Configuration::get('BLOCKSPECIALS_NB_CACHES'));
	}
	...
	//_tat_ добавим поддержку в классе продукта
