{capture name=path}<a href="{smartblog::GetSmartBlogLink('smartblog')|escape:"html":"UTF-8"}">{l s='All Blog News' mod='smartblog'}</a>{*<span class="navigation-pipe">{$navigationPipe}</span>*}{$meta_title}{/capture}
<div id="content" class="block">
   <div itemtype="https://schema.org/Article" itemscope="" id="sdsblogArticle" class="blog-post">
   		<div class="page-item-title">
   			<h1 itemprop="name">{$meta_title}</h1>
   		</div>
      <div class="post-info">
         {assign var="catOptions" value=null}
                     ...
                     <span>
               {l s='Posted by ' mod='smartblog'} {if $smartshowauthor ==1}&nbsp;<i class="icon icon-user"></i><span itemprop="author">{if $smartshowauthorstyle != 0}{$firstname} {$lastname}{else}{$lastname} {$firstname}{/if}</span>&nbsp;<i class="icon icon-calendar"></i>&nbsp;<span itemprop="dateCreated">{$created|date_format}</span>{/if}&nbsp;&nbsp;<i class="icon icon-tags"></i>&nbsp;<span itemprop="articleSection"><a href="{smartblog::GetSmartBlogLink('smartblog_category',$catOptions)|escape:"html":"UTF-8"}">{$title_category}</a></span> &nbsp;<i class="icon icon-comments"></i>&nbsp; {if $countcomment != ''}{$countcomment}{else}{l s='0' mod='smartblog'}{/if}{l s=' Comments' mod='smartblog'}</span>
