{*
* 2007-2014 PrestaShop _tat_ тут во избежание путаницы с первым модулем поменяла layered_form на layered_pxp_form. Но главное в теме
*
* NOTICE OF LICENSE
*
...
<div id="layered_block_left" class="block">
	<h4 class="title_block">{l s='Catalog' mod='blockpxplay'}</h4>
	<div class="block_content">
		<form action="#" class="layered_pxp_form">
			<div>
				{if isset($selected_filters) && $n_filters > 0}
				<div id="enabled_filters">
					<span class="layered_subtitle" style="float: none;">{l s='Enabled filters:' mod='blockpxplay'}</span>
					<ul>
								...
								{if $filter.type == $filter_type && isset($filter.values)}
									{if isset($filter.slider)}
										{if $smarty.foreach.f_values.first}
											<li>
												<a href="#" data-rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='blockpxplay'}">x</a>
												{if $filter.format == 1}
									...
									{else}
										{foreach from=$filter.values key=id_value item=value}
											{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
												<li>
													<a href="#" data-rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blockpxplay'}">x</a>
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						...
						{else}
							{if $filter.filter_type == 0}
								<label for="{$filter.type}">{l s='Range:' mod='blockpxplay'}</label> <span id="layered_{$filter.type}_range"></span>
							...
							{else}
								{if $filter.filter_type == 1}
								<li class="nomargin">
									{l s='From' mod='blockpxplay'} <input class="layered_{$filter.type}_range layered_input_range_min layered_input_range" id="layered_{$filter.type}_range_min" type="text" value="{$filter.values[0]}"/>
									<span class="layered_{$filter.type}_range_unit">{$filter.unit}</span>
									{l s='to' mod='blockpxplay'} <input class="layered_{$filter.type}_range layered_input_range_max layered_input_range" id="layered_{$filter.type}_range_max" type="text" value="{$filter.values[1]}"/>
									<span class="layered_{$filter.type}_range_unit">{$filter.unit}</span>
	...
	<div id="layered_ajax_loader" style="display: none;">
		<p><img src="{$img_ps_dir}loader.gif" alt="" /><br />{l s='Loading...' mod='blockpxplay'}</p>
	</div>
</div>
{else}
<div id="layered_block_left" class="block">
	...
	<div style="display: none;">
		<p style=""><img src="{$img_ps_dir}loader.gif" alt="" /><br />{l s='Loading...' mod='blockpxplay'}</p>
	</div>
</div>
{/if}
<!-- /Block layered navigation module -->
