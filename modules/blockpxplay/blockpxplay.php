<?php
/*
* 2007-2014 PrestaShop _tat_ тут соединение с БД заменила в названии таблиц layered на pxplay
*
* NOTICE OF LICENSE
*
...
class blockpxplay extends Module
{
	private $products;
	...
	{
		$this->name = 'blockpxplay';
		$this->tab = 'front_office_features';
		$this->version = '2.0.7';
		$this->author = 'PrestaShop_tat';
		$this->need_instance = 0;
		$this->bootstrap = true;

		parent::__construct();

		$this->displayName = $this->l('Filtr for rxr');
		$this->description = $this->l('Filtr for rxr. Show pxp');
	...
	public function install()
	{
		...
		&& $this->registerHook('postProcessAttribute') && $this->registerHook('afterDeleteAttribute') && $this->registerHook('afterSaveAttribute') && $this->registerHook('displayProductListReviews'))
		{
			if (version_compare(_PS_VERSION_, '1.6.0', '>=') === true)
			{
				...
				if ((!$theme->default_left_column || !$this->registerHook('leftColumn'))
					&& (!$theme->default_right_column || !$this->registerHook('rightColumn')) || !$this->registerHook('displayProductListReviews'))
				{
				...
				$this->registerHook('productListReviews');
	...
	public function uninstall()
	{
		...
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_price_index');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_friendly_url');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_indexable_attribute_group');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_indexable_feature');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_indexable_attribute_lang_value');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_indexable_attribute_group_lang_value');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_indexable_feature_lang_value');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_indexable_feature_value_lang_value');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_category');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_filter');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_filter_shop');
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_product_attribute');
	...
	private static function installPriceIndexTable()
	{
		...
		CREATE TABLE `'._DB_PREFIX_.'pxplay_price_index` (
	...
	private function installFriendlyUrlTable()
	{
		Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pxplay_friendly_url`');
		Db::getInstance()->execute('
		CREATE TABLE `'._DB_PREFIX_.'pxplay_friendly_url` (
		...
		Db::getInstance()->execute('CREATE INDEX `url_key` ON `'._DB_PREFIX_.'pxplay_friendly_url`(url_key(5))');
	}
	...
	private function installIndexableAttributeTable()
	{
		...
		Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pxplay_indexable_attribute_group`');
		Db::getInstance()->execute('
		CREATE TABLE `'._DB_PREFIX_.'pxplay_indexable_attribute_group` (
		`id_attribute_group` INT NOT NULL,
		`indexable` BOOL NOT NULL DEFAULT 0,
		PRIMARY KEY (`id_attribute_group`)
		) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=utf8;');
		Db::getInstance()->execute('
		INSERT INTO `'._DB_PREFIX_.'pxplay_indexable_attribute_group`
		SELECT id_attribute_group, 1 FROM `'._DB_PREFIX_.'attribute_group`');

		Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pxplay_indexable_attribute_group_lang_value`');
		Db::getInstance()->execute('
		CREATE TABLE `'._DB_PREFIX_.'pxplay_indexable_attribute_group_lang_value` (
		`id_attribute_group` INT NOT NULL,
		`id_lang` INT NOT NULL,
		`url_name` VARCHAR(20),
		...
		Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pxplay_indexable_attribute_lang_value`');
		Db::getInstance()->execute('
		CREATE TABLE `'._DB_PREFIX_.'pxplay_indexable_attribute_lang_value` (
		...
		Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pxplay_indexable_feature`');
		Db::getInstance()->execute('
		CREATE TABLE `'._DB_PREFIX_.'pxplay_indexable_feature` (
		...
		Db::getInstance()->execute('
		INSERT INTO `'._DB_PREFIX_.'pxplay_indexable_feature`
		SELECT id_feature, 1 FROM `'._DB_PREFIX_.'feature`');

		Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pxplay_indexable_feature_lang_value`');
		Db::getInstance()->execute('
		CREATE TABLE `'._DB_PREFIX_.'pxplay_indexable_feature_lang_value` (
		`id_feature` INT NOT NULL,
		`id_lang` INT NOT NULL,
		...
		Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pxplay_indexable_feature_value_lang_value`');
		Db::getInstance()->execute('
		CREATE TABLE `'._DB_PREFIX_.'pxplay_indexable_feature_value_lang_value` (
	...
	public function installProductAttributeTable()
	{
		Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'pxplay_product_attribute`');
		Db::getInstance()->execute('
		CREATE TABLE `'._DB_PREFIX_.'pxplay_product_attribute` (
	...
	public function hookAfterSaveAttributeGroup($params)
	{
		...
		Db::getInstance()->execute(
			'DELETE FROM '._DB_PREFIX_.'pxplay_indexable_attribute_group
			WHERE `id_attribute_group` = '.(int)$params['id_attribute_group']
		);
		Db::getInstance()->execute(
			'DELETE FROM '._DB_PREFIX_.'pxplay_indexable_attribute_group_lang_value
			WHERE `id_attribute_group` = '.(int)$params['id_attribute_group']
		);

		Db::getInstance()->execute(
			'INSERT INTO '._DB_PREFIX_.'pxplay_indexable_attribute_group (`id_attribute_group`, `indexable`)
		...
		foreach (Language::getLanguages(false) as $language)
		{
			...
			Db::getInstance()->execute(
				'INSERT INTO '._DB_PREFIX_.'pxplay_indexable_attribute_group_lang_value
	...
	public function hookAttributeGroupForm($params)
	{
		...
		$is_indexable = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
			'SELECT `indexable`
			FROM '._DB_PREFIX_.'pxplay_indexable_attribute_group
			WHERE `id_attribute_group` = '.(int)$params['id_attribute_group']
		);
		...
		if ($result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
			'SELECT `url_name`, `meta_title`, `id_lang` FROM '._DB_PREFIX_.'pxplay_indexable_attribute_group_lang_value
			WHERE `id_attribute_group` = '.(int)$params['id_attribute_group']
		))
	...
	public function hookAfterSaveAttribute($params)
	{
		...
		Db::getInstance()->execute(
			'DELETE FROM '._DB_PREFIX_.'pxplay_indexable_attribute_lang_value
		...
		foreach (Language::getLanguages(false) as $language)
		{
			...
			Db::getInstance()->execute(
				'INSERT INTO '._DB_PREFIX_.'pxplay_indexable_attribute_lang_value
	...
	public function hookAttributeForm($params)
	{
		...
		if ($result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
			...
			FROM '._DB_PREFIX_.'pxplay_indexable_attribute_lang_value
			WHERE `id_attribute` = '.(int)$params['id_attribute']
		))
			foreach ($result as $data)
				$values[$data['id_lang']] = array('url_name' => $data['url_name'], 'meta_title' => $data['meta_title']);
	...
	public function hookAfterSaveFeature($params)
	{
		...
		Db::getInstance()->execute(
			'DELETE FROM '._DB_PREFIX_.'pxplay_indexable_feature
			WHERE `id_feature` = '.(int)$params['id_feature']
		);
		Db::getInstance()->execute(
			'DELETE FROM '._DB_PREFIX_.'pxplay_indexable_feature_lang_value
			WHERE `id_feature` = '.(int)$params['id_feature']
		);
		...
		foreach (Language::getLanguages(false) as $language)
		{
			...
			Db::getInstance()->execute(
				'INSERT INTO '._DB_PREFIX_.'pxplay_indexable_feature_lang_value
	...
	public function hookFeatureForm($params)
	{
		...
		$is_indexable = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
			'SELECT `indexable`
			FROM '._DB_PREFIX_.'pxplay_indexable_feature
			WHERE `id_feature` = '.(int)$params['id_feature']
		);
		...
		if ($result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
			'SELECT `url_name`, `meta_title`, `id_lang` FROM '._DB_PREFIX_.'pxplay_indexable_feature_lang_value
			WHERE `id_feature` = '.(int)$params['id_feature']
		))
			foreach ($result as $data)
				$values[$data['id_lang']] = array('url_name' => $data['url_name'], 'meta_title' => $data['meta_title']);
	...
	public function hookAfterSaveFeatureValue($params)
	{
		...
		Db::getInstance()->execute(
			'DELETE FROM '._DB_PREFIX_.'pxplay_indexable_feature_value_lang_value
		...
		foreach (Language::getLanguages(false) as $language)
		{
			...
			Db::getInstance()->execute(
				'INSERT INTO '._DB_PREFIX_.'pxplay_indexable_feature_value_lang_value
	...
	public function hookFeatureValueForm($params)
	{
		...
		if ($result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
			'SELECT `url_name`, `meta_title`, `id_lang`
			FROM '._DB_PREFIX_.'pxplay_indexable_feature_value_lang_value
			WHERE `id_feature_value` = '.(int)$params['id_feature_value']
		))
			foreach ($result as $data)
				$values[$data['id_lang']] = array('url_name' => $data['url_name'], 'meta_title' => $data['meta_title']);
	...
	public function hookProductListAssign($params)
	{
		...
		$categories_count = Db::getInstance()->getValue('
			SELECT COUNT(*)
			FROM '._DB_PREFIX_.'pxplay_category
	...
	public function hookDisplayProductListReviews($id_product,$params)	
	{	
	...
	public function hookHeader($params)
	{
		...
		$this->context->controller->addJS(($this->_path).'blockpxplay.js');
		...
		if (version_compare(_PS_VERSION_, '1.6.0', '>=') === true)
			$this->context->controller->addCSS(($this->_path).'blockpxplay.css', 'all');
		else
			$this->context->controller->addCSS(($this->_path).'blockpxplay-15.css', 'all');
		$this->context->controller->addJQueryPlugin('scrollTo');

		$filters = $this->getSelectedFilters();

		// Get non indexable attributes
		$attribute_group_list = Db::getInstance()->executeS('SELECT id_attribute_group FROM '._DB_PREFIX_.'pxplay_indexable_attribute_group WHERE indexable = 0');
		// Get non indexable features
		$feature_list = Db::getInstance()->executeS('SELECT id_feature FROM '._DB_PREFIX_.'pxplay_indexable_feature WHERE indexable = 0');
	...
	public function hookFooter($params)
	{
		...
		if (Dispatcher::getInstance()->getController() == 'category')
			$this->context->controller->addJS($this->_path.'blockpxplay-footer.js');
	}
	...
	public function hookCategoryDeletion($params)
	{
		$layered_filter_list = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
			'SELECT * FROM '._DB_PREFIX_.'pxplay_filter'
		);
		...
		foreach ($layered_filter_list as $layered_filter)
		{
			...
			if (in_array((int)$params['category']->id, $data['categories']))
			{
				...
				Db::getInstance()->execute(
					'UPDATE `'._DB_PREFIX_.'pxplay_filter`
	...
	public function indexAttribute($id_product = null)
	{
		if (is_null($id_product))
			Db::getInstance()->execute('TRUNCATE '._DB_PREFIX_.'pxplay_product_attribute');
		else
			Db::getInstance()->execute('
				DELETE FROM '._DB_PREFIX_.'pxplay_product_attribute
				WHERE id_product = '.(int)$id_product
			);

		Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'pxplay_product_attribute` (`id_attribute`, `id_product`, `id_attribute_group`, `id_shop`)
	...
	{
		if ($truncate)
			Db::getInstance()->execute('TRUNCATE '._DB_PREFIX_.'pxplay_friendly_url');
		...
		$filters = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM '._DB_PREFIX_.'pxplay_category lc
			...
			switch ($filter['type'])
			{
				case 'id_attribute_group':
					$attributes = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
						...
						LEFT JOIN '._DB_PREFIX_.'pxplay_indexable_attribute_group liag ON (liag.id_attribute_group = a.id_attribute_group)
						LEFT JOIN '._DB_PREFIX_.'pxplay_indexable_attribute_group_lang_value liagl
						ON (liagl.id_attribute_group = ag.id_attribute_group AND liagl.id_lang = '.(int)$filter['id_lang'].')
						LEFT JOIN '._DB_PREFIX_.'pxplay_indexable_attribute_lang_value lial
				...
				case 'id_feature':
					$features = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
						...
						LEFT JOIN '._DB_PREFIX_.'pxplay_indexable_feature lif ON (lif.id_feature = fl.id_feature)
						...
						LEFT JOIN '._DB_PREFIX_.'pxplay_indexable_feature_lang_value lifl
						ON (lifl.id_feature = fl.id_feature AND lifl.id_lang = '.(int)$filter['id_lang'].')
						LEFT JOIN '._DB_PREFIX_.'pxplay_indexable_feature_value_lang_value lifvl
			...
			foreach ($attribute_values as $attribute)
				foreach ($attribute as $param)
				{
					...
					$id_layered_friendly_url = Db::getInstance()->getValue('
						...
						FROM `'._DB_PREFIX_.'pxplay_friendly_url` WHERE `id_lang` = '.$id_lang.' AND `url_key` = \''.$url_key.'\''
					);

					if ($id_layered_friendly_url == false)
					{
						Db::getInstance()->AutoExecute(_DB_PREFIX_.'pxplay_friendly_url', array('url_key' => $url_key, 'data' => serialize($selected_filters), 'id_lang' => (int)$id_lang), 'INSERT');
	...
	private static function indexPrices($cursor = null, $full = false, $ajax = false, $smart = false)
	{
		if ($full)
			...
			$nb_products = (int)Db::getInstance()->getValue('
				...
				INNER JOIN `'._DB_PREFIX_.'product_shop` ps
					ON (ps.`id_product` = p.`id_product` AND ps.`active` = 1 AND ps.`visibility` IN ("both", "catalog"))
				LEFT JOIN  `'._DB_PREFIX_.'pxplay_price_index` psi ON (psi.id_product = p.id_product)
		...
		{
			$token = substr(Tools::encrypt('blockpxplay/index'), 0, 10);
			if (Tools::usingSecureMode())
				$domain = Tools::getShopDomainSsl(true);
			else
				$domain = Tools::getShopDomain(true);

			if (!Tools::file_get_contents($domain.__PS_BASE_URI__.'modules/blockpxplay/blockpxplay-price-indexer.php?token='.$token.'&cursor='.(int)$cursor.'&full='.(int)$full))
	...
	public static function indexProductPrices($id_product, $smart = true)
	{
		...
		foreach ($shop_list as $id_shop)
		{
			...
			if ($smart)
				Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'pxplay_price_index` WHERE `id_product` = '.(int)$id_product.' AND `id_shop` = '.(int)$id_shop);
			...
			Db::getInstance()->execute('
				INSERT INTO `'._DB_PREFIX_.'pxplay_price_index` (id_product, id_currency, id_shop, price_min, price_max)
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('SubmitFilter'))
		{
			...
			else
			{
				...
				{
					Db::getInstance()->execute('
						DELETE FROM '._DB_PREFIX_.'pxplay_filter
				...
				{
					Db::getInstance()->execute('TRUNCATE TABLE '._DB_PREFIX_.'pxplay_filter');
					$categories = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
						SELECT id_category
				...
				Db::getInstance()->execute('
					DELETE FROM '._DB_PREFIX_.'pxplay_filter_shop
				...
				if (count($_POST['categoryBox']))
				{
					...
					Db::getInstance()->autoExecute(_DB_PREFIX_.'pxplay_filter', $values_to_insert, 'INSERT');
					$id_layered_filter = (int)Db::getInstance()->Insert_ID();

					if (isset($assos))
					foreach ($assos as $asso)
						Db::getInstance()->execute('
							INSERT INTO '._DB_PREFIX_.'pxplay_filter_shop (`id_layered_filter`, `id_shop`)
		...
		else if (Tools::getValue('deleteFilterTemplate'))
		{
			$layered_values = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
				SELECT filters
				FROM '._DB_PREFIX_.'pxplay_filter
				WHERE id_layered_filter = '.(int)Tools::getValue('id_layered_filter')
			);
		...
		{
			$this->context->controller->addJS($this->_path.'js/blockpxplay_admin.js');
		...
		if (version_compare(_PS_VERSION_, '1.6.0', '>=') === true)
			$this->context->controller->addCSS($this->_path.'css/blockpxplay_admin_1.6.css');
		else
			$this->context->controller->addCSS($this->_path.'css/blockpxplay_admin.css');

		if (Tools::getValue('add_new_filters_template'))
		{
			$this->context->smarty->assign(array(
				'current_url' => $this->context->link->getAdminLink('AdminModules').'&configure=blockpxplay&tab_module=front_office_features&module_name=blockpxplay',
				'uri' => $this->getPathUri(),
				'id_layered_filter' => 0,
		...
		else
		{
			$this->context->smarty->assign(array(
				...
				'price_indexer_url' => $module_url.'blockpxplay-price-indexer.php'.'?token='.substr(Tools::encrypt('blockpxplay/index'), 0, 10),
				'full_price_indexer_url' => $module_url.'blockpxplay-price-indexer.php'.'?token='.substr(Tools::encrypt('blockpxplay/index'), 0, 10).'&full=1',
				'attribute_indexer_url' => $module_url.'blockpxplay-attribute-indexer.php'.'?token='.substr(Tools::encrypt('blockpxplay/index'), 0, 10),
				'url_indexer_url' => $module_url.'blockpxplay-url-indexer.php'.'?token='.substr(Tools::encrypt('blockpxplay/index'), 0, 10).'&truncate=1',
				'filters_templates' => Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT * FROM '._DB_PREFIX_.'pxplay_filter ORDER BY date_add DESC'),
	...
	private function getSelectedFilters()
	{
		...
		if (strpos($_SERVER['SCRIPT_FILENAME'], 'blockpxplay-ajax.php') === false || Tools::getValue('selected_filters') !== false)
		{
			...
			if (!empty($url_attributes))
			{
				foreach ($url_attributes as $url_attribute)
				{
					...
					else
					{
						...
						{
							$data = Db::getInstance()->getValue('SELECT data FROM `'._DB_PREFIX_.'pxplay_friendly_url` WHERE `url_key` = \''.md5('/'.$attribute_name.$this->getAnchor().$url_parameter).'\'');
							if ($data)
	...
	public function getProductByFilters($selected_filters = array())
	{
		...
		if (!count($selected_filters['category']))
		{
			if (Configuration::get('PS_LAYERED_FULL_TREE'))
				...
				AND c.active = 1)
				RIGHT JOIN '._DB_PREFIX_.'pxplay_category lc ON (lc.id_category = '.(int)$id_parent.' AND
		...
		{
			$price_filter_query_in = 'INNER JOIN `'._DB_PREFIX_.'pxplay_price_index` psi
			ON
			(
			...
			$price_filter_query_out = 'INNER JOIN `'._DB_PREFIX_.'pxplay_price_index` psi
			ON
	...
	public function getFilterBlock($selected_filters = array())
	{
		...
		$filters = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT * FROM '._DB_PREFIX_.'pxplay_category
		...
		foreach ($filters as $filter)
		{
			...
			switch ($filter['type'])
			{
				...
				case 'id_attribute_group':// attribute group
					...
					$sql_query['from'] = '
					FROM '._DB_PREFIX_.'pxplay_product_attribute lpa
			...
			foreach ($filters as $filter_tmp)
			{
				...
				if (method_exists('blockpxplay', $method_name) &&
		...
		LEFT JOIN `'._DB_PREFIX_.'pxplay_indexable_attribute_group` liag
		...
		SELECT name
		FROM `'._DB_PREFIX_.'feature_lang` fl
		LEFT JOIN  `'._DB_PREFIX_.'pxplay_indexable_feature` lif
	...
	public function generateFiltersBlock($selected_filters)
	{
		...
		if ($filter_block = $this->getFilterBlock($selected_filters))
		{
			...
			$smarty->assign(array(
				...
				'blockpxplaySliderName' => $translate,
				'col_img_dir' => _PS_COL_IMG_DIR_
			));
			return $this->display(__FILE__, 'blockpxplay.tpl');
		}
		else
			return false;
	}
	...
	private static function getPriceFilterSubQuery($filter_value)
	{
		...
		{
			$price_filter_query = '
			INNER JOIN `'._DB_PREFIX_.'pxplay_price_index` psi ON (psi.id_product = p.id_product AND psi.id_currency = '.(int)$id_currency.'
			AND psi.price_min <= '.(int)$filter_value[1].' AND psi.price_max >= '.(int)$filter_value[0].' AND psi.id_shop='.(int)Context::getContext()->shop->id.') ';
		}
		else
		{
			$price_filter_query = '
			INNER JOIN `'._DB_PREFIX_.'pxplay_price_index` psi
	...
	public function ajaxCall()
	{
		...
		if ($nb_products == 0)
			$product_list = $this->display(__FILE__, 'blockpxplay-no-products.tpl');
		else
	...
	public function rebuildLayeredStructure()
	{
		...
		Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'pxplay_category');
		Db::getInstance()->execute('
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'pxplay_category` (
		...
		Db::getInstance()->execute('
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'pxplay_filter` (
		...
		Db::getInstance()->execute('
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'pxplay_filter_shop` (
	...
	public function rebuildLayeredCache($products_ids = array(), $categories_ids = array())
	{
		...
		if ($to_insert)
		{
			Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'pxplay_filter(name, filters, n_categories, date_add)
				VALUES (\''.sprintf($this->l('My template %s'), date('Y-m-d')).'\', \''.pSQL(serialize($filter_data)).'\', '.count($filter_data['categories']).', NOW())');

			$last_id = Db::getInstance()->Insert_ID();
			Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'pxplay_filter_shop WHERE `id_layered_filter` = '.$last_id);
			foreach ($shop_list as $id_shop)
				Db::getInstance()->execute('INSERT INTO '._DB_PREFIX_.'pxplay_filter_shop (`id_layered_filter`, `id_shop`)
	...
	public function buildLayeredCategories()
	{
		...
		$res = Db::getInstance()->executeS('SELECT * FROM '._DB_PREFIX_.'pxplay_filter ORDER BY date_add DESC');
		$categories = array();
		// Remove all from layered_category
		Db::getInstance()->execute('TRUNCATE '._DB_PREFIX_.'pxplay_category');

		if (!count($res)) // No filters templates defined, nothing else to do
			return true;

		$sql_to_insert = 'INSERT INTO '._DB_PREFIX_.'pxplay_category (id_category, id_shop, id_value, type, position, filter_show_limit, filter_type) VALUES ';
		$values = false;
