...
*
* NOTICE OF LICENSE _tat_ Вывод товаров в корзину. В процессе подключаются shopping-cart-product-line.tpl, js. В частности убрала вывод обоих Дублей адреса доставки. Сверху, в адресной строке выводится "order"
*
...
<h3 id="cart_title" class="page-heading">
		...
		<table id="cart_summary" class="table table-bordered {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
			<thead>
				<tr>
					...
					{/if}
					<th class="cart_unit item text-right">{l s='Unit price'}</th>{*Заголовок - цена за единицу*}
			...
			<tfoot>
				...
				<tr class="cart_total_price5">
					...
					{else}
						<td class="price" class="total_price_container"></td>
						<td class="price" class="total_price_container"></td>
						<td class="price" class="total_price_container"></td>
						<td class="price" id="total_price_container">
							<span id="total_price">{displayPrice price=$total_price_without_tax}</span>{*Итого, к оплате: - Цифра*}
						</td>
					{/if}
				</tr>
			</tfoot>
