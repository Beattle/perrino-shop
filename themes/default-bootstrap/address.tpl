{*
* 2007-2015 PrestaShop _tat_ Добавление адреса. Поменяла классы
* PrestaShop _tat_  Страница главной уч. записи. Заменила form-group на formAdress Последняя ссылка добавляется в my-account.tpl
* NOTICE OF LICENSE
*
	...
	<form action="{$link->getPageLink('address', true)|escape:'html':'UTF-8'}" method="post" class="std" id="add_address">
		...
		{foreach from=$ordered_adr_fields item=field_name}
			{if $field_name eq 'company'}
				<div class="formAdress" hidden><!-- Компания -->
                ...
                <div id="vat_area">
                    <div id="floor">
                        <div class="form-group">
                            <label for="floor">{l s='Floor'}</label>
                            <input type="text" class="form-control validate" data-validate="{$address_validation.$field_name.validate}" id="floor" name="floor" value="{if isset($smarty.post.floor)}{$smarty.post.floor}{else}{if isset($address->floor)}{$address->floor|escape:'html':'UTF-8'}{/if}{/if}" />
                        </div>
                    </div>
                </div>
            {/if}
			...
			{if $field_name eq 'address2'}
				<div class="required formAdress" hidden><!-- Продолжение адреса -->
			...
			{if $field_name eq 'postcode'}
				...
				<div class="required postcode formAdress unvisible">
			...
			{if $field_name eq 'city'}
				<div class="required formAdress"><!-- Город -->
			...
			{if $field_name eq 'Country:name' || $field_name eq 'country'}
				<div class="required formAdress" hidden><!-- Страна -->
			...
			{if $field_name eq 'State:name'}
				...
				<div class="required id_state formAdress">
			...
			<!-- Добавленный лифт -->
				<!--{assign var="liftExist" value=true} Наверное не надо -->
				<div class="formAdress">
			...
			{if $field_name eq 'phone'}
				...
				<div class="formAdress phone-number">
		...
		{/if}
		{if !$stateExist}
			<div class="required id_state formAdress unvisible">
				<label for="id_state">{l s='State'} <sup>*</sup></label>
				<select name="id_state" id="id_state" class="form-control">
					<option value="">-</option>
				</select>
		...
		{/if}
		{if !$dniExist}
			<div class="required dni formAdress unvisible">
				<label for="dni">{l s='Identification number'} <sup>*</sup></label>
		...
		{if !$mobilePhoneExist}
			...
			<div class="clearBotch"></div>
		{/if}
		<div class="required formAdress" id="adress_alias"><!-- Название адреса -->
