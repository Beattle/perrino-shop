{*
* 2007-2015 PrestaShop _tat_ Здесь убрала вывод в корзину скидки и старой цены. После заказа прямо из корзины эти величины пересчитывались в cart-summary.js и снова появлялись) Плюс меняла вид кнопок плюс и минус (корзина))
*
* NOTICE OF LICENSE
*
	...
	<td class="cart_unit" data-title="{l s='Unit price'}">
		<ul class="price text-right" id="product_price_{$product.id_product}_{$product.id_product_attribute}{if $quantityDisplayed > 0}_nocustom{/if}_{$product.id_address_delivery|intval}{if !empty($product.gift)}_gift{/if}">
			...
			{else}
				...
				{/if}
				{*_tat_ Здесь убрала скидку и старую цену*}
			{/if}
		</ul>
	</td>
