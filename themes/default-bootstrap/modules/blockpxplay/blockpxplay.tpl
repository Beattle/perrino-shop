...
*
* NOTICE OF LICENSE _tat_ Выводит блок расширенных фильтров. Тут во избежание путаницы с первым модулем поменяла layered_form на layered_pxp_form.
*
...
<!-- _tat_ Эти файлы подключались в FrontController.php Но тогда не работала авторизация А это фильтры и комменты-->	
<script src="{$base_dir}/themes/default-bootstrap/js/autoload/10-bootstrap.min.js"></script>
<script src="{$base_dir}/themes/default-bootstrap/js/autoload/15-jquery.total-storage.min.js"></script>
<script src="{$base_dir}/themes/default-bootstrap/js/autoload/15-jquery.uniform-modified.js"></script>
</head>
...
<div id="layered_block_left" class="block">	
	<div class="left_filter block_content"><!-- был block_content -->
		<form action="#" class="layered_pxp_form">
			<div>
				{if isset($selected_filters) && $n_filters > 0}
				<div id="enabled_filters">
					<span class="layered_subtitle" style="float: none;">
						{l s='Enabled filters:' mod='blockpxplay'}
					</span>
					<ul>
									...
									{if $filter.type == $filter_type && isset($filter.values)}
										{if isset($filter.slider)}
											{if $smarty.foreach.f_values.first}
												<li>
													<a href="#" data-rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='blockpxplay'}"></a>
													{if $filter.format == 1}
														{l s='%1$s: %2$s - %3$s'|sprintf:$filter.name:{displayPrice price=$filter.values[0]}:{displayPrice price=$filter.values[1]}|escape:'html':'UTF-8' mod='blockpxplay'}
													{else}
														{l s='%1$s: %2$s %4$s - %3$s %4$s'|sprintf:$filter.name:$filter.values[0]:$filter.values[1]:$filter.unit|escape:'html':'UTF-8' mod='blockpxplay'}
													{/if}
												</li>
											{/if}
										{else}
											{foreach from=$filter.values key=id_value item=value}
												{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
													<li>
														<a href="#" data-rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blockpxplay'}"><i class="icon-remove"></i></a>
														{l s='%1$s: %2$s' mod='blockpxplay' sprintf=[$filter.name, $value.name]}
													</li>
												{/if}
											{/foreach}
										{/if}
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						...
						{/if}
                            <!-- <p>{$filter.name|escape:'html':'UTF-8'}</p><!-- имя фильтра -->
                            <p>{$filter.name|escape:'html':'UTF-8'}</p>				
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
								...
								{else}
									...
									{else}<!-- Блок селектов -->
										<select class="select form-control" {if $filter.filter_show_limit > 1}multiple="multiple" size="{$filter.filter_show_limit}"{/if}>
											<option value="">{l s='No filters' mod='blockpxplay'}</option>
											{foreach from=$filter.values key=id_value item=value}
											{if $value.nbr || !$hide_0_values}
												<option style="color: {if isset($value.color)}{$value.color}{/if}" id="layered_{$filter.type_lite}{if $id_value || $filter.type == 'quantity'}_{$id_value}{/if}" value="{$id_value}_{$filter.id_key}" {if isset($value.checked) && $value.checked}selected="selected"{/if} {if !$value.nbr}disabled="disabled"{/if}><!-- Здесь присваивается класс selected селекту-->
													{$value.name|escape:'html':'UTF-8'}{if $layered_show_qties}<span> ({$value.nbr})</span>{/if}
												</option>
											{/if}
											{/foreach}
								...
								{else}
									...
									{foreach from=$filter.list_of_values  item=values}
										<li class="nomargin {if $filter.values[1] == $values[1] && $filter.values[0] == $values[0]}layered_list_selected{/if} layered_list" onclick="$('#layered_{$filter.type}_range_min').val({$values[0]});$('#layered_{$filter.type}_range_max').val({$values[1]});reloadContent();">
											- {l s='From' mod='blockpxplay'} {$values[0]} {$filter.unit} {l s='to' mod='blockpxplay'} {$values[1]} {$filter.unit}
										</li>
									{/foreach}
							...
							{if count($filter.values) > $filter.filter_show_limit && $filter.filter_show_limit > 0 && $filter.filter_type != 2}
								<span class="hide-action more">{l s='Show more' mod='blockpxplay'}</span>
	...
	<div id="layered_ajax_loader" style="display: none;">
		<p>
			...
			<br />{l s='Loading...' mod='blockpxplay'}
		</p>
	</div>
</div>
</div><!-- cat_size -->
