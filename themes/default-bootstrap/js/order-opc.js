...
$(document).ready(function(){
	...
	//alert("KK"); При оформлении заказа
...
//				$('#opc_account_form').slideDown('slow');
		...
		$(document).on('click', '#submitAccount, #submitGuestAccount', function(e){
			...
			$('#opc_account_form input:visible, #opc_account_form input[type=hidden],#firstname, #lastname').each(function() {
				...
				else if ($(this).is('input[type=radio]'))
				{
                    ...
                    if($(this).attr('id') == 'firstname' ){
                        if($(this).val() == '' || $(this).val() == undefined){
                            $(this).val($('#customer_firstname').val());
                            params += encodeURIComponent($(this).attr('name')+'='+encodeURIComponent($(this).val()))+'&';
                        }
                    ...
                    if($(this).attr('id') == 'lastname' ){
                        if($(this).val() == '' || $(this).val() == undefined){
                            $(this).val($('#customer_lastname').val());
                            params += encodeURIComponent($(this).attr('name')+'='+encodeURIComponent($(this).val()))+'&';
                        }

                        //$(this).attr('id') == 'lastname'
                    }
                    if($(this).attr('id') == 'email' && ($(this).val() == '' || $(this).val == undefined)){
                        params += encodeURIComponent($(this).attr('name'))+'='+encodeURIComponent($(this).val('asotsky@gmail.com'))+'&';
                    }
					params += encodeURIComponent($(this).attr('name'))+'='+encodeURIComponent($(this).val())+'&';
			});
	...
	//		params += 'is_new_customer='+encodeURIComponent($('#is_new_customer').val())+'&';
			...
			$.ajax({
				...
				success: function(jsonData)
				{
					...
					/** Do not show payment_methods cause we have only 1 **/
                    //$('#opc_new_account-overlay, #opc_delivery_methods-overlay, #opc_payment_methods-overlay').fadeIn('slow');
                     $.ajax({
                        type:'GET',
                        dataType:'json',
                        url:'/module/cashondelivery/validation',
                        data:{'confirm':1,'ajax':true},
                        cache:false,
                        success:function(data){
                           // var order = decodeURIComponent(data);
                            url = data.replace('.php','');
                            window.location = '/index.php?controller='+url;
...
function saveAddress(type)
{
    ...
    params += 'floor=' + encodeURIComponent($('#floor' + (type == 'invoice' ? '_invoice' : '')).val()) + '&';
    params += 'lift='  + encodeURIComponent($('#lift' + (type == 'invoice' ? '_invoice' : '')).val()) + '&';
    params += 'code_intercom='  + encodeURIComponent($('#code_intercom' + (type == 'invouce' ? '_invoice' : '')).val()) + '&';
