/*
* 2007-2015 PrestaShop _tat_ Щелчок по селекту фильтра _tat_ тут во избежание путаницы с первым модулем поменяла layered_form на layered_pxp_form. самый главный файл перезагрузки фильтра и ещё кое что
*
* NOTICE OF LICENSE
*
...
$(document).ready(function()
{
	...
	$(document).on('click', '#layered_pxp_form .select', function(e) {
		...
		//setTimeout(function() {
		//$(".select :nth-child(1)").attr("selected", "selected");//Обнуляю значение левого фильтра
		//alert($(this).attr('class'));
		reloadpxpContent(true);//Начинается перезагрузка контента
		
		//}, 1000);
	});
	
	//$('.select').click(function (e){
		//alert($('#layered_pxp_form .select').val($(this).val()));
		//alert("a");
		//reloadpxpContent(true);//Начинается перезагрузка контента
	//}); 
	...
	$(document).off('change').on('change', 'select[name=n]', function(e) 
	{
		...
		//alert($('select[name=n]').val($(this).val()));
...
function addpxpSlider(type, data, unit, format)
{
	sliderList.push({
		type: type,
		data: data,
...
function reloadpxpContent(params_plus)//Перезагрузка контента
{
	...
	ajaxQuery = $.ajax(
	{
		...
		success: function(result)
		{
			...
			paginationpxpButton(result.nbRenderedProducts, result.nbAskedProducts);
			ajaxLoaderOn = 0;
