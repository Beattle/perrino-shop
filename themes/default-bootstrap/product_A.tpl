...
{if $errors|@count == 0}
 ...
 <div class="product_block">
								...
								{foreach from=$groups key=id_attribute_group item=group}
									{if $group.attributes|@count}
										<fieldset class="attribute_fieldset">
											<span {if $group.group_type != 'color' && $group.group_type != 'radio'}for="group_{$id_attribute_group|intval}"{/if}>{$group.name|escape:'html':'UTF-8'}&nbsp;</span>
							...
							<!-- prices -->
								...
								<div{if (!$product->specificPrice || !$product->specificPrice.reduction) && $group_reduction == 0} class="hidden"{/if}>
									...
									{if $priceDisplay >= 0 && $priceDisplay <= 2}
										<strike><span>{hook h="displayProductPriceBlock" product=$product type="old_price"}</span></strike><!-- конец не попадаю -->
										<strike><span id="old_price_display">{if $productPriceWithoutReduction > $productPrice}{convertPrice price=$productPriceWithoutReduction}{/if}</span></strike>
										<!-- процент скидки -->
			...
			{*Вывожу характеристики в нужном порядке*}
				...
				{/if}
					{if isset($typeMatressTitle) && $typeMatressTitle}
						<div>
							<a href="http://perrino-shop.ru/content/3-States#typeMatrass"><p>{$typeMatressTitle}</p>
				...
				{/if}				
					{if isset($naFullMatrasTitle) && $naFullMatrasTitle}
							<div>
								<a href="http://perrino-shop.ru/content/3-States#typeMatrass"><p>{$naFullMatrasTitle}</p>
						...
						{/if}
							{if isset($typePruzinTitle) && $typePruzinTitle}
								<div>
									<a href="http://perrino-shop.ru/content/3-States#typeMatrass"><p>{$typePruzinTitle}</p>
									<span>{$typePruzin}</span></a>
								</div>						
							{/if}
								{if isset($hardnessTitle) && $hardnessTitle}
									<div>
										<a href="http://perrino-shop.ru/content/3-States#typeMatrass"><p>{$hardnessTitle}</p>
								...
								{/if}								
									{if isset($hardnessDopTitle) && $hardnessDopTitle}
										<div>
											<a href="http://perrino-shop.ru/content/3-States#typeMatrass"><p>{$hardnessDopTitle}</p>
											<span>{$hardnessDop}</span></a>
										</div>						
									{/if}	
										{if isset($heightTitle) && $heightTitle}{*Матрас*}
											<div>
												<a href="http://perrino-shop.ru/content/3-States#typeMatrass"><p>{$heightTitle}</p>
												<span>{$height}</span></a>
											</div>
										{/if}
										...
										{/if}
											{if isset($vesTitle) && $vesTitle}
												<div>
													<a href="http://perrino-shop.ru/content/3-States#typeMatrass"><p>{$vesTitle}</p>
													<span>{$ves}</span></a>
												</div>
											{/if}
	...
	{if isset($product) && $product->description}
					<!-- full description -->
					<div  class="descriptorTovar">{$product->description}</div>
		{/if} 
	{if !$content_only}
