{*
* 2007-2015 PrestaShop _tat_ Кажется через свой контроллер и сам регулирует нужные подключения нужных файлов корзины и заказа при нужных настройках
*
* NOTICE OF LICENSE
*
...
{else}
	{if $productNumber}
		...
		{if $is_logged AND !$is_guest}
			{include file="$tpl_dir./order-address.tpl"}<!-- add new addres-->
		{else}
			<!-- Create account / Guest account / Login block -->
			{include file="$tpl_dir./order-opc-new-account.tpl"}
			<!-- END Create account / Guest account / Login block -->
		{/if}
		<!-- Carrier Перевозчик. Подключает файл заказа выводящий перевозчика -->
		{*include file="$tpl_dir./order-carrier.tpl"*}
		<!-- END Carrier -->
	
		<!-- Payment Подключает файл заказа выводящий вид оплаты -->
		{*include file="$tpl_dir./order-payment.tpl"*}
		<!-- END Payment -->
	{else}
		{capture name=path}{l s='Your shopping cart'}{/capture}
		<h2 class="page-heading">{l s='Your shopping cart'}</h2><!-- Шаг. Тут - перевозчики -->
...
<script type="text/javascript">
    (function(w, p) {
        ...
        (w[p] = w[p] || []).push({
            ...
            tag: '235cd9ee7fa961057475d9b4f80270f5'
        });
